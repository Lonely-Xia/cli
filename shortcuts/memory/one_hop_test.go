...
func TestGraphOneHopMetadata(t *testing.T) {
	if MemoryGraphOneHop.Service != "memory" || MemoryGraphOneHop.Command != "+graph-one-hop" {
		t.Fatalf("shortcut = %#v, want memory +graph-one-hop", MemoryGraphOneHop)
	}
	if MemoryOneHop.Service != "memory" || MemoryOneHop.Command != "+one-hop" {
		t.Fatalf("compat shortcut = %s %s, want memory +one-hop", MemoryOneHop.Service, MemoryOneHop.Command)
	}
	if want := []string{"user"}; !reflect.DeepEqual(MemoryGraphOneHop.AuthTypes, want) {
		t.Fatalf("AuthTypes = %#v, want %#v", MemoryGraphOneHop.AuthTypes, want)
	}
