...
先按请求查找必要的事实和素材，再判断输出类型与写作场景。将 Memory 用于信息组织、格式、措辞和语气；不要把 Memory 当作该请求的事实来源。若请求要求创建或修改文档，应将写作风格实际应用到文档，并按请求执行创建或原位修改，完成后回读。若没有要求文档操作，按指定形式直接回答。`,
...
{写作风格 memory}`,
		...
		DocumentSteps: []string{
			...
			"按文档类型选择匹配的场景化风格；需要参考格式且 Memory 提供同类型文档时，只选最相关的一份，并实际读取其完整代表性区块和真实 block 结构。",
