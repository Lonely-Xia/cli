...
// MemoryGraphOneHop returns the timeline nodes and one-hop neighbours for one or
...
var MemoryGraphOneHop = common.Shortcut{
	Service:     memoryService,
	Command:     "+graph-one-hop",
...
func parseGraphOneHopSpec(input graphOneHopInput, now time.Time, generatedTraceID string) (graphOneHopSpec, error) {
	...
	if input.Format != "json" && input.Format != "pretty" {
		return graphOneHopSpec{}, common.ValidationErrorf("--format must be json or pretty for memory +graph-one-hop").
			WithParam("--format")
	}
...
// MemoryOneHop is a hidden compatibility alias for older installed skills.
var MemoryOneHop = deprecatedShortcutAlias(MemoryGraphOneHop, memoryService, "+one-hop", "memory +graph-one-hop")
...
func executeGraphOneHop(rctx *common.RuntimeContext, spec graphOneHopSpec) error {
	result, err := callGraphOneHop(rctx, spec)
	if err != nil {
		return err
	}
	var renderErr error
	rctx.OutFormat(result.Data, result.Meta, func(w io.Writer) {
		renderErr = renderGraphOneHopPretty(w, result.Data, result.Meta)
	})
	return renderErr
}

type graphOneHopCallResult struct {
	Data map[string]interface{}
	Meta *output.Meta
}

func callGraphOneHop(rctx *common.RuntimeContext, spec graphOneHopSpec) (*graphOneHopCallResult, error) {
	return callGraphOneHopWithTTEnv(rctx, spec, "")
}

func callGraphOneHopWithTTEnv(rctx *common.RuntimeContext, spec graphOneHopSpec, ttEnv string) (*graphOneHopCallResult, error) {
	req := &larkcore.ApiReq{
	...
	if err != nil {
		return nil, annotateGraphOneHopError(err)
	}
	...
	return &graphOneHopCallResult{Data: out, Meta: meta}, nil
}
